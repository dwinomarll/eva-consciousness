 ...
 *   - bundled Ma'at MCP installs and builds
 *
...
const ALL = ['filesystem', 'memory', 'sequentialthinking', 'gmail', 'gcal', 'slack', 'maat', 'notion', 'omi']
...
try {
  ...
  console.log("installing and building bundled Ma'at MCP...")
  await execa('npm', ['install'], { cwd: path.join(node, 'maat-mcp'), stdio: 'inherit' })
  await execa('npm', ['run', 'build'], { cwd: path.join(node, 'maat-mcp'), stdio: 'inherit' })
